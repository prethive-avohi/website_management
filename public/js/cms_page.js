...
function _set_primary_action(frm) {
    ...
    if (frm.doc.content_json) {
        // Content exists → show Preview as primary
    ...
    } else if (frm.doc.source_file && !_is_generating(frm)) {
        // File attached but not yet generated → show Generate as primary
...
function _render_generation_banner(frm) {
    ...
    const banners = {
        ...
        "Failed":     ["red",   `<span>✗ <b>Generation failed.</b> See AI Error Log below. Fix the issue and click <b>AI → Generate Content</b> to retry.</span>`],
    };
...
function _set_status_colour(frm) {
    ...
    const colour = map[frm.doc?.generation_status] || "gray";
