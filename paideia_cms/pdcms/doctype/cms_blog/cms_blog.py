...
from paideia_cms.cms.services.cache_service import invalidate_blog, invalidate_registry
...
class CMSBlog(Document):
    ...
    def after_insert(self):
        if self.source_file:
            frappe.enqueue(
                "paideia_cms.jobs.ai_generation.run_blog_generation",
                queue="long",
                timeout=600,
                blog_name=self.name,
            )
    ...
    def on_update(self):
        invalidate_blog(self.slug)
        invalidate_registry()
        if self.workflow_state == "Published" and self.has_value_changed("workflow_state"):
            trigger_deploy(f"CMS Blog '{self.slug}' published")

    def _maybe_create_slug_redirect(self):
        old_slug = frappe.db.get_value("CMS Blog", self.name, "slug")
        if not old_slug or old_slug == self.slug:
            return
        old_state = frappe.db.get_value("CMS Blog", self.name, "workflow_state")
