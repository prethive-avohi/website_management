...
class CMSTemplate(Document):
    ...
    def on_trash(self):
        ...
        # Clears template link on all content before Frappe's link check runs.
        # Pages/Blogs/Courses survive with template=null.
        for doctype in ("CMS Page", "CMS Blog", "CMS Course"):
            linked = frappe.get_all(doctype, filters={"template": self.name}, pluck="name")
            for name in linked:
                frappe.db.set_value(doctype, name, "template", None)
