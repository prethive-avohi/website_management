...
function _add_menu_buttons(frm) {
    if (frm.doc.content_json) {
        frm.add_custom_button(__("Preview"), () => {
            window.open(`/api/method/paideia_cms.api.v1.preview.render_course?name=${encodeURIComponent(frm.docname)}`, "_blank");
        }, __("View"));
...
function _publish(frm) {
    frappe.confirm("Publish this course? It will be visible on the frontend immediately.", () => {
...
function _set_workflow_state(frm, state) {
    frappe.call({
        ...
        args: { doctype: "CMS Course", name: frm.docname, fieldname: "workflow_state", value: state },
        callback(r) {
            if (!r.exc) {
                frm.reload_doc();
                frappe.show_alert({ message: state === "Published" ? "Course published." : "Course unpublished.", indicator: state === "Published" ? "green" : "orange" });
            }
        },
    });
}
